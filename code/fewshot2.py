...
def execute_few_shot(questions_per):
    ...
    """
    course_location = 'C:/Users/ADMIN/Desktop/mathQ/code/csv/hw2_result.csv'
    ...
    for i in range(questions_per):
        ...
        if results.iloc[i]['Zero-Shot Evaluation'] == 1:
            print('no few shot needed for ' + 'hw2' + ' question ' + str(i+1))
            few_shot_input = 'n/a'
            few_shot_output = 'n/a'
...
def execute_GPT3_CoT_one_shot(questions_per):
    """
    Runs one-shot CoT on questions_per questions for each course in courses.
    """
    course_location = 'hw2' + ' results.csv'
    #initializing new columns in csv
    ...
    for i in range(questions_per):
        ...
        print("Running GPT-3 CoT one-shot on " + 'hw2' + ' question ' + str(i+1) + '...')
        start = time.time()
